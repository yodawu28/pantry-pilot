...
- [ ] Agent workflow:
  - [ ] Receive image → extract metadata → extract items → validate → return JSON
  - [ ] Retry logic for low-confidence extractions
  - [ ] Error handling for tool failures
  - [ ] **Agent returns JSON only** - API handles database save independently
...
- [ ] Update receipt upload endpoint:
  - [ ] Trigger agent extraction after image upload
  - [ ] Option: async task queue (Celery/RQ) or simple background task
  - [ ] Update receipt status to 'processing'
- [ ] New endpoints:
  - [ ] `POST /receipts/{id}/ocr` — manually trigger extraction
    - [ ] Fetch image from MinIO
    - [ ] Call Gemma-3n agent with image
    - [ ] Agent returns JSON with extracted data
    - [ ] API independently saves to database
  - [ ] `PATCH /receipts/{id}` — update parsed fields (merchant, total, etc.)
  - [ ] `GET /receipts/{id}/ocr-status` — check extraction processing status
- [ ] Add validation:
...
4. **Extraction Strategy:**
   ...
   - Multi-pass: Metadata → Items → Validation → Return JSON (more accurate)
   - **Recommendation:** Multi-pass with MCP tools for better accuracy and debugging
   - **Note:** Agent returns JSON only; API handles database persistence independently
