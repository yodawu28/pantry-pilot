...
async def validate_extraction(
    ...
    if metadata.merchant_name:
        ...
        if not re.match(r'^[A-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂÂÊÔƠƯ\s\-\'\.&0-9]+$', metadata.merchant_name, re.I):
            warnings.append(f"Merchant name '{metadata.merchant_name}' contains special characters.")
    else:
    ...
    if metadata.total_amount:
        ...
        if metadata.currency == "VND" and metadata.total_amount > 50000000: # 50 triệu
            ...
            warnings.append(f"Total amount unusually high for {metadata.currency} (>{metadata.total_amount}).")
    else:
    ...
    if not items:
        ...
        for idx, item in enumerate(items):
            ...
            if abs(expected_item_total - Decimal(str(item.total_price))) > Decimal('500'): # Ngưỡng 500đ
                warnings.append(
